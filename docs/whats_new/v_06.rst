...
Major Changes:
    ...
    - **Added functionality**: the ``ArbitraryImputer`` can now take a dictionary of variable, arbitrary number pairs, to impute different variables with different numbers (by Michał Gromiec).
...
Major Changes:
    ...
    - **Renamed**: the ``AddNaNBinaryImputer`` is now called ``MissingIndicator``.
