...
Missing data refers to the absence of observed values in a dataset and is a common occurrence in real-world data
science projects.

In data science, missing data can lead to biased analysis, inaccurate predictions, and reduced reliability
of models. Hence, handling missing data is the first, and one of the most important steps in a data preprocessing pipeline.

Feature-engine supports several imputation techniques to handle missing data.

In the following paragraphs, we first introduce different types of missing data mechanisms,
and after that provide an overview of each of the supported missing data imputation methods.
...
Missing data imputation refers to the process of estimating and replacing missing values in a dataset. It involves
...
We can select the imputation value in one of 3 ways depending on the variable's distribution:
...
- For simplicity, the value can be set at a multiple of the maximum value.
...
Tree based models can be effectively trained on data imputed with end tail imputation.
...
imputation is used for categorical variable imputation.
...
‘Missing’ or the string 'NaN' for example. In essence, it consists of treating the missing observations as a category in itself.
