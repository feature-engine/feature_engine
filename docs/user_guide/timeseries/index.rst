...
Feature engine's :class:`~forecasting.LagFeatures` implements lag features. These are straightforward to 
...
:class:`~forecasting.WindowFeatures`, also known as rolling window features, are used to summarize past behavior over a 
...
`feature selection transformers <https://feature-engine.trainindata.com/en/latest/api_doc/selection/index.html>`_
...
:class:`~forecasting.ExpandingWindowFeatures` are similar to rolling window features but instead 
...
minute etc. directly from the datetime variable using :class:`DateTimeFeatures`. These features 
are important to identify seasonal patterns, daily trends, especially when certain time periods 
...
Feature-engine allows us to capture this behavior through the :class:`CyclicalFeatures` transformer.
