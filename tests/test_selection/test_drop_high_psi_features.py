...
def test_check_psi_values():
    ...
    test=DropHighPSIFeatures(
        df2, n_bins=2, 
        method='equal_width', switch_basis=False, 
...
def test_switch_basis():
    ...
    case=DropHighPSIFeatures(df_a, n_bins=5, switch_basis=False, min_pct_empty_buckets = 0.001)
    case.fit(df_b)

    switch_case=DropHighPSIFeatures(df_a, n_bins=5, switch_basis=True, min_pct_empty_buckets = 0.001)
    switch_case.fit(df_b)

    reverse_case=DropHighPSIFeatures(df_b, n_bins=5, switch_basis=True, min_pct_empty_buckets = 0.001)
...
def test_split_df_according_to_time():
    ...
    cut_off = DropHighPSIFeatures(basis, n_bins=5, switch_basis=sb, min_pct_empty_buckets = 0.001)
    cut_off.fit(df).psi

    separate_df = DropHighPSIFeatures(below_value, n_bins=5, switch_basis=sb, min_pct_empty_buckets = 0.001)
