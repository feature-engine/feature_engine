...
def test_sklearn_imputer_numeric_with_constant(dataframe_na):
    ...
    na_variables_left_after_imputation = [col
                                          for col
                                          in dataframe_na.loc[:, dataframe_na.isna().any()].columns
                                          if col not in variables_to_impute]

    transformer = SklearnTransformerWrapper(transformer=SimpleImputer(fill_value=-999, strategy='constant'),
    ...
    # transformed output
    assert all(dataframe_na_transformed[na_variables_left_after_imputation].isna().sum() != 0)
...
def test_sklearn_imputer_object_with_constant(dataframe_na):
    ...
    transformer = SklearnTransformerWrapper(transformer=SimpleImputer(fill_value='missing', strategy='constant'),
                                            variables=variables_to_impute)
