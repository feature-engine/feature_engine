...
def test_defo_params_plus_automatically_find_variables(dataframe_enc_big):
...
def test_user_provides_grouping_label_name_and_variable_list(dataframe_enc_big):
...
def test_raises_error_if_tol_not_between_0_and_1():
    with pytest.raises(ValueError):
        encoder = RareLabelEncoder(tol=5)


def test_raises_error_if_n_categories_not_int():
    with pytest.raises(ValueError):
        encoder = RareLabelEncoder(n_categories=0.5)


def test_raises_error_if_replace_with_not_string():
    with pytest.raises(ValueError):
        encoder = RareLabelEncoder(replace_with=0.5)


def test_raises_warning_if_variable_cardinality_less_than_n_categories(dataframe_enc_big):
...
def test_fit_raises_error_if_df_contains_na(dataframe_enc_big_na):
...
def test_transform_raises_error_if_df_contains_na(dataframe_enc_big, dataframe_enc_big_na):
...
def test_user_provides_max_n_categories(dataframe_enc_big):
