...
def test_encode_categories_in_k_binary_plus_select_vars_automatically(dataframe_enc_big):
...
def test_encode_categories_in_k_minus_1_binary_plus_list_of_variables(dataframe_enc_big):
...
def test_encode_top_categories(dataframe_enc_big):
...
def test_raises_error_if_top_categories_not_integer():
    with pytest.raises(ValueError):
        OneHotEncoder(top_categories=0.5)


def test_raises_error_if_drop_last_not_bool():
    with pytest.raises(ValueError):
