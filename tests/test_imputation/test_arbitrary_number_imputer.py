...
def test_imputer_with_99_and_automatically_select_variables(dataframe_na):
...
def test_imputer_with_1_and_single_variable_entered_by_user(dataframe_na):
    # set up transformer
...
def test_imputer_raises_error_when_arbitrary_number_is_string():
    with pytest.raises(ValueError):
        ArbitraryNumberImputer(arbitrary_number='arbitrary')


def test_imputer_raises_non_fitted_error(dataframe_na):
    with pytest.raises(NotFittedError):
        imputer = ArbitraryNumberImputer()
        imputer.transform(dataframe_na)


def test_imputer_with_dictionary_of_imputation_values(dataframe_na):
    # set up transformer
...
def imputer_raises_error_when_dictionary_value_is_string():
    with pytest.raises(ValueError):
