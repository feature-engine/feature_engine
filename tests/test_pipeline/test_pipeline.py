...
def test_pipeline_with_1_transformer(create_data):
    ...
    Xt, yt = pipe.fit_transform(X=X, y=y)
...
def test_with_2_transformers_from_fe(create_data):
    ...
    dmd = DropMissingData().fit(X)
    Xt1, yt1 = dmd.transform_x_y(X=X, y=y)
    Xt2 = OrdinalEncoder(encoding_method="arbitrary").fit_transform(Xt1, yt1)
    ...
    pipe.fit(X, y)
    Xt, yt = pipe.named_steps["drop"].transform_x_y(X, y)
    pd.testing.assert_frame_equal(Xt1, Xt)
    pd.testing.assert_series_equal(yt1, yt)
    Xt = pipe.named_steps["enc"].transform(Xt)
    pd.testing.assert_frame_equal(Xt2, Xt)
...
def test_with_1_transformers_from_sklearn(create_data):
    ...
    dmd = DropMissingData().fit(X)
    Xt1, yt1 = dmd.transform_x_y(X=X, y=y)
    Xt2 = sk_OrdinalEncoder().set_output(transform="pandas").fit_transform(Xt1)
    model = Lasso(random_state=10).fit(Xt2, yt1)
    preds = model.predict(Xt2)
