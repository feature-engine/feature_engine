...
from feature_engine.creation import BinaryCombinatioByFeatureReference


# test param variables_to_combine
def test_error_when_param_variables_not_entered():
    with pytest.raises(TypeError):
        BinaryCombinatioByFeatureReference()


def test_error_when_user_enters_one_variable():
    with pytest.raises(KeyError):
        BinaryCombinatioByFeatureReference(variables_to_combine=["Age"])


def test_error_when_variables_to_combine_wrong_type():
    with pytest.raises(ValueError):
        BinaryCombinatioByFeatureReference(variables_to_combine=("Age", "Name"))
    with pytest.raises(ValueError):
        BinaryCombinatioByFeatureReference(variables_to_combine=["Age", 0, 4.5])
    with pytest.raises(ValueError):
        BinaryCombinatioByFeatureReference(reference_variables=("Age", "Name"))
    with pytest.raises(ValueError):
        BinaryCombinatioByFeatureReference(reference_variables=["Age", 0, 4.5])

# test param binary_operations
def test_error_if_operation_not_supported():
    with pytest.raises(ValueError):
        BinaryCombinatioByFeatureReference(
            variables_to_combine=["Age", "Name"],reference_variables=["Age", "Name"], binary_operations=["an_operation"]
        )
...
def test_error_if_operation_is_wrong_type():
    with pytest.raises(ValueError):
        BinaryCombinatioByFeatureReference(
            variables_to_combine=["Age", "Name"], reference_variables=["Age", "Name"], binary_operations=[sum]
        )
    with pytest.raises(ValueError):
        BinaryCombinatioByFeatureReference(
            variables_to_combine=["Age", "Name"], reference_variables=["Age", "Name"], binary_operations=("sub", "div")
        )
...
def test_default_parameters(df_vartypes):
    transformer = BinaryCombinatioByFeatureReference(variables_to_combine=["Age", "Marks"])
...
def test_error_when_variables_to_combine_not_numeric(df_vartypes):
    transformer = BinaryCombinatioByFeatureReference(variables_to_combine=["Name", "Age", "Marks"])
    with pytest.raises(TypeError):
        transformer.fit_transform(df_vartypes)


def test_error_when_entered_variables_not_in_df(df_vartypes):
    transformer = BinaryCombinatioByFeatureReference(
...
def test_variable_names_when_df_cols_are_integers(df_numeric_columns):
    transformer = BinaryCombinatioByFeatureReference(variables_to_combine=[2, 3])
