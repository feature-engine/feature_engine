...
def test_gaussian_capping_right_tail_with_fold_1(dataframe_normal_dist):
...
def test_gaussian_capping_both_tails_with_fold_2(dataframe_normal_dist):
...
def test_iqr_capping_both_tails_with_fold_1(dataframe_normal_dist):
...
def test_iqr_capping_left_tail_with_fold_2(dataframe_normal_dist):
...
def test_quantile_capping_both_tails_with_fold_10_percent(dataframe_normal_dist):
...
def test_quantile_capping_both_tails_with_fold_15_percent(dataframe_normal_dist):
...
def test_transformer_ignores_na_in_df(dataframe_na):
...
def test_raises_error_if_capping_method_not_permitted():
    # test error raises
    with pytest.raises(ValueError):
        Winsorizer(capping_method='other')


def test_raises_error_if_tail_value_not_permitted():
    with pytest.raises(ValueError):
        Winsorizer(tail='other')


def test_raises_error_if_missing_values_not_permited():
    with pytest.raises(ValueError):
        Winsorizer(missing_values='other')


def test_raises_error_if_fold_value_not_permitted():
    with pytest.raises(ValueError):
        Winsorizer(fold=-1)


def test_raises_error_if_capping_method_quantiles_and_fold_value_not_permitted():
    with pytest.raises(ValueError):
        Winsorizer(capping_method='quantiles', fold=0.3)


def test_fit_raises_error_if_na_in_inut_df(dataframe_na):
...
def test_transform_raises_error_if_na_in_input_df(dataframe_vartypes, dataframe_na):
