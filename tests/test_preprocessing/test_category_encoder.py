...
from feature_engine.preprocessing import CategoryEncoder
...
def test_category_encoder_outputs_correct_dtype():
    ...
    res_str = CategoryEncoder().fit(df_str).transform(df_str)
    assert res_str.dtypes["col1"] == "category"

    df_float = pd.DataFrame({
        "col1": [1.0, 2.0, 3.0]
    })
    tr = CategoryEncoder(variables=["col1"], ignore_format=True)
    ...
    res_obj = CategoryEncoder().fit(df_obj).transform(df_obj)
    assert res_obj.dtypes["col1"] == "category"

    df_categ = pd.DataFrame({
        "col1": pd.Categorical(pd.Series(["a", "b", "c"]))
    })
    res_categ = CategoryEncoder().fit(df_categ).transform(df_categ)
...
def test_category_encoder_handles_missing():
    ...
    with pytest.raises(ValueError):
        tr = CategoryEncoder(errors="raise").fit(df_nas)
    tr = CategoryEncoder(errors="raise").fit(df_no_nas)
    with pytest.raises(ValueError):
        tr.transform(df_nas)

    # check that it doens't fail for missing values when using 'ignore'
    tr = CategoryEncoder(errors="ignore").fit(df_nas)
    with pytest.warns(UserWarning):
        tr.transform(df_nas)

    # check that it doesn't fail at transforming new values when using 'ignore'
    tr = CategoryEncoder(errors="ignore").fit(df_no_nas)
    with pytest.warns(UserWarning):
        tr.transform(df_new)
...
def test_category_outputs_correct_results():
    ...
    tr = CategoryEncoder(variables=["col1", "col2"], ignore_format=True).fit(df)
    res = tr.transform(df)
