...
class DateTimeBaseTransformer(BaseEstimator, TransformerMixin):
    ...
    def _select_variables_from_dict(
        ...
        """
        Checks that input is a dataframe, checks that variables in the dictionary
        entered by the user are of type datetime.
        ...
        TypeError
            If the input is not a Pandas DataFrame or a numpy array
            If any of the variables in the dictionary are not datetime
        ValueError
            If there are no numerical variables in the df or the df is empty
            If the variable(s) contain null values
        ...
        variables = [x for x in user_dict_.keys()]
        self.variables_ = _find_or_check_datetime_variables(X, variables)
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> pd.DataFrame:
        ...
        TypeError
            If the input is not a Pandas DataFrame or a numpy array
            If any of the user provided variables are not or cannot be converted
            to datetime
        ValueError
            If there are no datetime variables in the df or the df is empty
            If the variable(s) contain null values
